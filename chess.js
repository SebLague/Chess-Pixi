...
class Piece {
    constructor(sprite, colour, squareIndex) {
        this.sprite = sprite;
        this.colour = colour;
...
function squareIndexFromPoint(p) {
...
function posFromSquareIndex(squareIndex) {
    return new Point((squareIndex.x+.5) * size,(7.5-squareIndex.y)*size);
}
...
function onPieceReleased() {
    let squareIndex = squareIndexFromPoint(selectedSprite.position);
    let pos = posFromSquareIndex(squareIndex);
